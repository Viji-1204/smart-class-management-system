    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className={`${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white flex flex-col transition-transform duration-300 ease-in-out`}>
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => { setActiveTab('performance'); setMobileMenuOpen(false); }} className={`w-full flex items-center space-x-3 p-3 rounded-lg transition-colors ${activeTab === 'performance' ? 'bg-blue-600' : 'hover:bg-slate-800'}`}>
            <BarChart3 size={20} />
            <span>Academic Performance</span>
          </button>
          <button onClick={() => { setActiveTab('profile'); setMobileMenuOpen(false); }} className={`w-full flex items-center space-x-3 p-3 rounded-lg transition-colors ${activeTab === 'profile' ? 'bg-blue-600' : 'hover:bg-slate-800'}`}>
            <UserCircle size={20} />
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="p-4 lg:p-8">
          <header className="mb-6 lg:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="text-slate-500 text-sm lg:text-base">Hi, {user.name}</div>
          </header>
